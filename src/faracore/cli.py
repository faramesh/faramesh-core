...
def _get_base_url(args: argparse.Namespace) -> str:
    ...
    host = args.host if hasattr(args, 'host') and args.host else os.getenv("FARA_API_HOST", "127.0.0.1")
    port = args.port if hasattr(args, 'port') and args.port else int(os.getenv("FARA_API_PORT", "8000"))
    # Check if full URL is in env
    api_base = os.getenv("FARA_API_BASE")
    if api_base:
        return api_base
...
def _get_auth_token(args: argparse.Namespace) -> Optional[str]:
    ...
    if hasattr(args, 'token') and args.token:
        return args.token
    return os.getenv("FARA_AUTH_TOKEN")
...
"""
    ...
    print("  serve            Start the FaraCore server")
    ...
    print("[Examples]")
    print("  faracore list")
    print("  faracore list --full")
    print("  faracore list --json")
    print("  faracore get 2755d4a8")
    print("  faracore get 2755d4a8 --json")
    print("  faracore approve 2755d4a8")
    print("  faracore curl 2755d4a8")
    print("  faracore tail")
    print("  faracore logs 2755d4a8")
    print("  faracore --host localhost --port 9000 list")
    print("")
...
def make_parser():
    p = argparse.ArgumentParser(
        prog="faracore",
        description="FaraCore - Agent Action Governor CLI",
    ...
    # Server command
    p_serve = sub.add_parser("serve", help="Start the FaraCore server")
...
def cmd_init(args):
    ...
    default_content = """# FaraCore Default Policy
...
"""
    ...
    # Create .env.example
    env_example_content = """# FaraCore Configuration
# Copy this file to .env and customize as needed

# Server Configuration
FARACORE_HOST=127.0.0.1
FARACORE_PORT=8000
FARACORE_TOKEN=
...
# FARA_POSTGRES_DSN=postgresql://user:pass@localhost/faracore
...
"""
    ...
    if HAS_RICH:
        ...
        if created:
            ...
            console.print("  3. Run: [bold]faracore serve[/bold]")
        else:
...
def cmd_build_ui(args):
    ...
    try:
        ...
        # After build, copy assets into src/faracore/web so the API
        # can serve them without any extra wiring.
        build_dir = web_dir / "dist"
        target_dir = package_root / "src" / "faracore" / "web"
        ...
        if HAS_RICH:
            console.print("[green]✓ UI built and assets copied into src/faracore/web[/green]")
        else:
            print("✓ UI built and assets copied into src/faracore/web")
    except subprocess.CalledProcessError as e:
...
def cmd_doctor(args):
    ...
    # Check database
    try:
        from faracore.server.storage import get_store
        store = get_store()
        # Try to connect
    ...
    # Check token
    token = os.getenv("FARACORE_TOKEN") or os.getenv("FARA_AUTH_TOKEN")
    if token:
        if HAS_RICH:
        ...
        warnings.append("No auth token configured (FARACORE_TOKEN or FARA_AUTH_TOKEN)")
    
    # Check UI assets (optional)
    package_root = Path(__file__).resolve().parents[2]
    web_build = package_root / "web" / "dist"
    web_src = package_root / "src" / "faracore" / "web"
    ...
    if web_build.exists() or web_src.exists():
        ...
        warnings.append("UI assets not found. Run 'faracore build-ui' to build.")
...
services:
  ...
  #     - faracore
  #   environment:
  #     - FARA_API_BASE=http://faracore:8000
"""
...
"""
    ...
    if not env_example.exists():
        ...
        env_content = """# FaraCore Configuration
# Copy this file to .env and customize as needed
...
def cmd_serve(args):
    ...
    host = args.host or os.getenv("FARACORE_HOST") or settings.api_host or "127.0.0.1"
    port = args.port or (int(os.getenv("FARACORE_PORT")) if os.getenv("FARACORE_PORT") else None) or settings.api_port or 8000
    
    # Hot reload policy if --hot-reload flag or --watch (deprecated) or FARACORE_HOT_RELOAD env var
    hot_reload_enabled = (
        args.hot_reload or 
        args.watch or 
        os.getenv("FARACORE_HOT_RELOAD") == "1"
    )
                ...
                def on_modified(self, event):
                    if event.src_path == str(self.policy_path):
                        try:
                            ...
                            from faracore.server.main import policies
    ...
    print(f"Starting FaraCore server on http://{host}:{port}")
    if hot_reload_enabled and is_local_policy:
        print("Policy hot-reload enabled (--hot-reload or FARACORE_HOT_RELOAD=1)")
