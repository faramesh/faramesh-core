# src/faracore/server/policy_engine.py
...
class PolicyEngine:
    ...
    def _match_condition(self, match: Dict, tool: str, operation: str, params: Dict, context: Dict) -> bool:
        ...
        if "contains" in match:
            probe = json.dumps(params) if isinstance(params, dict) else str(params)
            if match["contains"] not in probe:
                return False
        ...
        if "path_contains" in match:
            ...
            if match["path_contains"] not in str(path):
                return False

        if "path_starts_with" in match:
            path = self._get_param_value(params, context, "path", "")
            if not str(path).startswith(match["path_starts_with"]):
                return False

        if "path_ends_with" in match:
            path = self._get_param_value(params, context, "path", "")
            if not str(path).endswith(match["path_ends_with"]):
                return False

        # HTTP method matching
        if "method" in match:
            method = self._get_param_value(params, context, "method", "").upper()
            if method != match["method"].upper():
                return False
