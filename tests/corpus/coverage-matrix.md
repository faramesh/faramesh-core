...
| `runtime-core/linux-interception` | wip | C | faramesh-runtime | no | yes | yes | yes | yes | yes | no | no | no | - | no | Governed enforcement report matrix on Linux only; non-Linux hosts skip the harness (exit 0). Not a tier-A WAL + hook_truth agent row.; Some CI kernels may terminate the child under strict seccomp; harness tolerates selected exit codes when validating the report. |
