...
from .client import FaraCoreError, FaraCorePolicyError, _make_request, _get_config
...
def validate_policy_file(path: str) -> None:
    ...
    Raises:
        FaraCorePolicyError: If policy is invalid
    ...
    if not file_path.exists():
        raise FaraCoreError(f"Policy file not found: {path}")
    ...
    try:
        ...
        raise FaraCoreError(f"Failed to read policy file: {e}")
    
    # Local validation: parse and check structure
    try:
        if yaml is None:
            raise FaraCoreError("PyYAML required for local policy validation. Install with: pip install pyyaml")
        
        with open(file_path, "r") as f:
            data = yaml.safe_load(f)
        
        if not isinstance(data, dict):
            raise FaraCorePolicyError("Policy must be a YAML/JSON object")
        
        # Check for basic structure
        if "rules" not in data:
            raise FaraCorePolicyError("Policy must have 'rules' field")
        
        if not isinstance(data["rules"], list):
            raise FaraCorePolicyError("Policy 'rules' must be a list")
        ...
        if errors:
            raise FaraCorePolicyError(f"Policy validation errors: {'; '.join(errors)}")
    
    except FaraCorePolicyError:
        raise
    except Exception as e:
        raise FaraCorePolicyError(f"Failed to validate policy: {e}")
...
def test_policy_against_action(policy_path: str, action: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Raises:
        FaraCoreError: If endpoint doesn't exist or on other errors
    ...
    if not file_path.exists():
        raise FaraCoreError(f"Policy file not found: {policy_path}")
