...
POLICY_PATH="${FARAMESH_LANGCHAIN_REAL_POLICY:-$CORE_DIR/policies/langchain_single_agent.yaml}"
...
IDP_PROVIDER="${FARAMESH_LANGCHAIN_REAL_IDP_PROVIDER:-okta}"
...
resolve_python() {
  ...
  local existing="$CORE_DIR/.tmp/langchain-e2e/.venv/bin/python"
  if [[ -x "$existing" ]]; then
    echo "$existing"
    return 0
  fi
  ...
  "$venv_dir/bin/pip" install --disable-pip-version-check --quiet "langchain>=1,<2" -e "$CORE_DIR/sdk/python"
...
"$PYTHON_BIN" "$WORKSPACE_DIR/demo_interactive_ai_agent.py" >"$AGENT_OUTPUT_PATH" 2>&1
...
cur.execute(
  "select count(*) from dpr_records where agent_id = ? and tool_id = ? and effect = ? and credential_brokered = 1 and credential_source = ?",
